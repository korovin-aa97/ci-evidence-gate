...
RUN_URL_RE = re.compile(r"/actions/runs/(?P<run_id>[0-9]+)(?:/job/[0-9]+)?(?:$|[/?#])")
...
class GitHubClient:
    ...
    def _get(self, path: str, query: dict[str, str] | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            raise ApiError(
                f"GitHub API returned HTTP {exc.code} for {path}: {body}"
            ) from exc
    ...
    def workflow_run(self, run_id: int) -> dict[str, Any]:
        ...
        return self._get(f"/repos/{owner}/{repository}/actions/runs/{run_id}")
...
def workflow_run_id(details_url: object) -> int | None:
    if not isinstance(details_url, str):
        return None
    match = RUN_URL_RE.search(details_url)
