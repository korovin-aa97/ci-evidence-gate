...
Last reviewed: 2026-08-30. Scope: v0.1.3.
...
| Threat | v0.1.3 control | Residual risk |
| --- | --- | --- |
...
| Evidence belongs to another commit or PR base | Check and workflow run must equal the full requested head SHA; pull-request associations must also equal the trusted event base/head pair. | SHA and association authenticity depend on GitHub/local object database. |
...
| API outage, truncation, rate limit | Errors, oversized responses, excessive same-name candidates and more than 1000 runs are invalid, never sufficient. Rate-limit reset hints are reported without immediate retry. | Availability failure blocks merge by design. |
...
| Token exfiltration to alternate host | API base URL must be HTTPS, must match GitHub-owned Action context, and cross-origin redirects are rejected before forwarding credentials. | A deployment that exposes a separate custom secret token to candidate workflow code remains unsafe. |
...
| Receipt path is a candidate symlink | Receipt is written to a temporary regular file and atomically replaces the destination. | Candidate-controlled parent-directory symlinks remain part of runner/workspace trust. |
| Receipt edited after evaluation | Receipt digest is an Action output, written with delimiter-safe command-file syntax. | Digest is not signed and both file/output share runner trust. Upload/sign externally if needed. |
...
The pinned Action checks repository, API URL and base/head inputs against
GitHub-owned context before constructing an authenticated API client. This
prevents a candidate workflow edit from redirecting the default token through
those inputs. It does not make a candidate-controlled workflow definition a
trusted invocation boundary: protect the workflow and manifest independently.
